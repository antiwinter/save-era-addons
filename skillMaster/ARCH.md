...
| File | Role |
|------|------|
| `tests/emu.lua` | Loads fake-wow, boots it via `fw.init("era")`, loads the addon from its .toc, seeds the world via `GM.SetTradeSkillLine` / `GM.SetBag`, fires `TRADE_SKILL_SHOW`, then clicks `DoAction` in a loop. Prints the plan, then reports reach/budget/waste/use-rate using `ns.db:price` + `fake-wow.world` stats. |
...
| `scripts/gen-data.lua` | `lua gen-data.lua <version>` (default `era`): boots fake-wow via `fw.init(version)`, then walks the schema through GM — for every skill `GM:GetRecipe` (inlined into the row) and `GM:GetPrice` for the output, its reagents, and its teach item — and emits `data/<version>/skills.lua` + `data/<version>/item_prices.lua`. |
